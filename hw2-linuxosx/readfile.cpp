...
// upinit= Transform::upvector(up, vec3(0,0,1));
upinit=up;
 fovy = values[9];
 //fovy=fovy;
		...
		//transfstack.push(mat4(1.0)); //duplicated by push should multiply the previous matrix
		...
		mat3 rM=Transform::rotate(degrees,axis); 
		mat4 M(rM[0].x,rM[0].y,rM[0].z,0,
			rM[1].x,rM[1].y,rM[1].z,0,
			rM[2].x,rM[2].y,rM[2].z,0,
			0,0,0,1
			);	
		rightmultiply( M, transfstack) ;
        ...
        // I include the basic push/pop code for matrix stacks
          ...
          transfstack.push(transfstack.top()); //duplicate the top
